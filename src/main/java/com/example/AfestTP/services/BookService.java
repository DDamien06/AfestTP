...
public class BookService {
    ...
    public Book getById(Long id) {
        return bookRepository.findById(id).orElseThrow(UnknownResourceException::new);
    }
    ...
    public Book update(Book book) {
        Book existingBook = bookRepository.findById(book.getBookId()).orElseThrow(UnknownResourceException::new);
        return bookRepository.save(existingBook);
    }

    public void delete(Long id) {
        Book book = bookRepository.findById(id).orElseThrow(UnknownResourceException::new);
        if (isAvailable(book)) {
